...
for ($i=1;$i<=$Contar;$i++){
	$_DATOS_EXCEL[$i]['anio'] = $objPHPExcel->getActiveSheet()->getCell('A'.$i)->getCalculatedValue();
	$_DATOS_EXCEL[$i]['trim']= $objPHPExcel->getActiveSheet()->getCell('B'.$i)->getCalculatedValue();
	$_DATOS_EXCEL[$i]['sector']= $objPHPExcel->getActiveSheet()->getCell('C'.$i)->getCalculatedValue();
	$_DATOS_EXCEL[$i]['valor']= $objPHPExcel->getActiveSheet()->getCell('D'.$i)->getCalculatedValue();
	$_DATOS_EXCEL[$i]['vp']= $objPHPExcel->getActiveSheet()->getCell('E'.$i)->getCalculatedValue();
...
foreach($_DATOS_EXCEL as $campo => $valor){
	$sql = "INSERT INTO ap1Ind4b VALUES ($id,";

	foreach ($valor as $campo2 => $valor2){

		$sql.= $valor2.",";

		//$campo2 == "fecha_mod" ? $sql.= $valor2.");" : $sql.= $valor2.",";
	}
