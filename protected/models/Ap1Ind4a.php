...
class Ap1Ind4a extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_periodo, anio, id_trimestre, tipo_serie, user_reg, user_mod', 'numerical', 'integerOnly'=>true),
			array('total_act, vp', 'numerical'),
			array('fecha_reg, fecha_mod', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_periodo, anio, id_trimestre, tipo_serie, total_act, vp, fecha_reg, fecha_mod, user_reg, user_mod', 'safe', 'on'=>'search'),
		);
	}
